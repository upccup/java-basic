...
public class MinHeapSortTest {
    ...
    public void heapSort() {
        int[] result = MinHeapSort.heapSort(new int[]{5, 2, 3, 1});
        assertEquals(5, result[0], 0);
        assertEquals(3, result[1], 0);
        assertEquals(1, result[3], 0);
        result = MinHeapSort.heapSort(new int[]{5, 1, 1, 2, 0, 0});
        assertEquals(5, result[0], 0);
        assertEquals(2, result[1], 0);
        assertEquals(0, result[result.length - 1], 0);
        result = MinHeapSort.heapSort(new int[]{-1, 2, -8, -10});
        assertEquals(-10, result[3], 0);
        assertEquals(2, result[0], 0);
        result = MinHeapSort.heapSort(
            new int[]{-74, 48, -20, 2, 10, -84, -5, -9, 11, -24, -91, 2, -71, 64, 63, 80, 28, -30, -58, -11, -44, -87, -22, 54, -74, -10, -55, -28, -46, 29, 10,
                ...
                -39});
        assertEquals(-96, result[result.length-1], 0);
    }
}